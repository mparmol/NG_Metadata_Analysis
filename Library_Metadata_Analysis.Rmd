...
We can use this method to focus in the next game to play according to this criteria, just but filtering out the games already completed at 100%.
...
This analysis will show the buying out tendency of owner's library, by showing what kind of games are more prevalent in the library according to genres and tags.
...
Now we can explore the data and plot de proportion of each genre (filteres only showing the most abundant combinations of genre)
...
Results show a biased distribution to Indie, action adventure games, being the former the most representative kind of games in the library.

And we can do the same for the tags in the games. There are many tags, we will only focus on the most abundant by filtering out the lest representative (total count less than 20% of the greater counting tag)
...
We can see interesting things, like there is a strong correlation between female protagonist, fantasy, adventure and great soundtrack games. Still interesting is that tags correlate to genre, with singleplayer in the first position.

We can explore the potential correlation between genre and time for finish and complete.

```{r}

subset_genre<-datos[datos$V11 %in% names(table(datos$V11)[table(datos$V11)>(0.1*max(table(datos$V11)))]),]
...
ggplot(subset_genre,aes(V11,log2(V3))) + geom_boxplot() + theme_bw() + theme(axis.text.x = element_text(angle=45, hjust=1)) 
```

Looking at the results we can see that games that are more prevalents in the library are the games that need less time to finish. Would it be the same to 100% complete?
...
ggplot(subset_genre,aes(V11,log2(V4))) + geom_boxplot() + theme_bw() + theme(axis.text.x = element_text(angle=45, hjust=1))
```
